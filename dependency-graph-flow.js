...
// Custom Node Component using HTM
...
function DependencyGraphApp() {
  ...
  const parsedTasks = useMemo(() => {
    ...
    if (atlas.title && atlas.title.toLowerCase().includes("conflict")) {
      tasks[2].depends_on.push("TSK-007");
    }
    ...
    <div style=${{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style=${{ 
        ...
        <div style=${{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          >
            ${uniqueAgents.map(a => html`<option key=${a} value=${a}>${a}</option>`)}
          </select>
        </div>
      ...
      ${(circularDependencies.length > 0 || conflictNodes.length > 0) && html`
        <div style=${{
          display: 'flex',
          ...
          ${circularDependencies.length > 0 && html`
            <div>⚠️ CIRCULAR DEP DETECTED: ${circularDependencies.join(', ')}</div>
          `}
          ${conflictNodes.length > 0 && html`
                ...
                return html`
                  <div key=${t.id} style=${{ 
                    ...
                    <div style=${{ flex: 1, display: 'flex', position: 'relative', height: '24px', background: 'rgba(255,255,255,0.02)', borderRadius: '4px' }}>
                      ${t.weeks.includes(1) && html`<div style=${{ position: 'absolute', left: '0%', width: '25%', height: '100%', background: styles.badge, opacity: 0.7 }} />`}
                      ${t.weeks.includes(2) && html`<div style=${{ position: 'absolute', left: '25%', width: '25%', height: '100%', background: styles.badge, opacity: 0.7 }} />`}
                      ${t.weeks.includes(3) && html`<div style=${{ position: 'absolute', left: '50%', width: '25%', height: '100%', background: styles.badge, opacity: 0.7 }} />`}
                      ${t.weeks.includes(4) && html`<div style=${{ position: 'absolute', left: '75%', width: '25%', height: '100%', background: styles.badge, opacity: 0.7 }} />`}
        ...
        ${selectedNode && html`
          <div style=${{
            ...
            <div style=${{ borderTop: '1px solid rgba(26,48,72,0.4)', paddingTop: '10px', display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '11px' }}>
              ...
              ${selectedNode.depends_on && selectedNode.depends_on.length > 0 && html`
                <div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (container) {
    ...
    root.render(
      html`<${ReactFlowProvider}><${DependencyGraphApp} /></${ReactFlowProvider}>`
    );
  }
});
