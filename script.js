...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function pollAtlasJob(jobId, progressIndicator, progressPercent, progressStatusText) {
        ...
        for (let attempt = 0; attempt < maxPolls; attempt++) {
            ...
            const response = await fetch(
                `/api/atlas?job_id=${encodeURIComponent(jobId)}`
            );
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    `Atlas API ${response.status}: ${errorText}`
                );
            }
            ...
            const completed =
                data.job?.completed_sections?.length || 0;

            const total =
                data.job?.total_sections || 8;
    ...
    function downloadAtlasZipClientSide(atlas) {
        if (typeof JSZip === "undefined") {
            showToast("JSZip library not loaded yet.", "error");
            return;
        }
        const zip = new JSZip();

        // 1. /docs/
        const genFiles = atlas.generated_files || {};
        let readme = genFiles["README.md"] || `# ${atlas.title} — Production Plan\n\nGenerated by DreamXV Atlas.`;

        let roadmap_md = genFiles["Roadmap.md"] || `# Development Roadmap\n\n`;
        if (!genFiles["Roadmap.md"] && atlas.roadmap) {
            atlas.roadmap.forEach(phase => {
                roadmap_md += `## ${phase.name}\n`;
        ...
        let tasks_md = genFiles["Tasks.md"] || `# Project Tasks\n\n`;
        if (!genFiles["Tasks.md"] && atlas.task_breakdown) {
            const tb = atlas.task_breakdown;
            tasks_md += `## Critical Path Tasks (MVP)\n`;
            (tb.critical_tasks || []).forEach(t => { tasks_md += `- [ ] ${t}\n`; });
            tasks_md += `\n## Optional Tasks\n`;
            (tb.optional_tasks || []).forEach(t => { tasks_md += `- [ ] ${t}\n`; });
            tasks_md += `\n## Future Expansion\n`;
            (tb.future_expansion || []).forEach(t => { tasks_md += `- [ ] ${t}\n`; });
        }

        let flow_md = genFiles["FlowMap.md"] || `# Production Workflow & Dependencies\n\n`;
        if (!genFiles["FlowMap.md"]) {
            flow_md += `## Step-by-Step Workflow\n`;
            (atlas.production_flow_map || atlas.flow_map || []).forEach((step, idx) => {
                flow_md += `${idx + 1}. ${step}\n`;
            });
            flow_md += `\n## Core Dependency Graph\n`;
            (atlas.dependency_map || []).forEach(dep => {
                flow_md += `- ${dep}\n`;
            });
        }

        let struct_md = genFiles["Structure.md"] || `# Project Directory Structure\n\n`;
        if (!genFiles["Structure.md"]) {
            struct_md += `Below is the folder and file layout designed for this project:\n\n\`\`\`\n`;
            struct_md += (atlas.project_structure || atlas.structure || []).join("\n");
            struct_md += `\n\`\`\`\n`;
        }

        zip.file("docs/README.md", readme);
        zip.file("docs/Roadmap.md", roadmap_md);
        zip.file("docs/FlowMap.md", flow_md);
        zip.file("docs/Tasks.md", tasks_md);
        zip.file("docs/Structure.md", struct_md);

        // 2. /project_structure/
        const structureList = atlas.project_structure || atlas.structure || [];
        structureList.forEach(path => {
            const cleanedPath = path.strip ? path.strip() : path;
            if (cleanedPath.endsWith("/")) {
                zip.folder(`project_structure/${cleanedPath}`);
            }
        });
        ...
        const metadata = {
            atlas_id: atlas.id,
            source_project_id: atlas.source_project_id,
            title: atlas.title,
        ...
        zip.generateAsync({ type: "blob" }).then(function (content) {
            const link = document.createElement("a");
            link.href = URL.createObjectURL(content);
            link.download = `${atlas.id}_atlas_production_kit.zip`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            showToast("Production Kit ZIP generated and downloaded successfully!", "success");
        }).catch(err => {
            console.error("ZIP Generation failed:", err);
            showToast("Failed to generate ZIP archive locally.", "error");
        });
    }
    ...
    if (downloadZipBtn) {
        downloadZipBtn.addEventListener("click", async () => {
            if (activeAtlasResult && activeAtlasResult.id) {
                if (activeAtlasResult.id.startsWith("atl_")) {
                    downloadAtlasZipClientSide(activeAtlasResult);
                } else {
                    // Try downloading from backend. If it fails (e.g. 404/not found), fallback to client-side!
                    try {
                        const checkRes = await fetch(`/api/atlas?atlas_id=${encodeURIComponent(activeAtlasResult.id)}`);
...
function initRoadmapSimulator(atlas) {
    ...
    const runSimulation = () => {
        ...
        // Use the actual task-hour estimates generated by Atlas.
        let totalTasks = 0;
        if (atlas.roadmap) {
            atlas.roadmap.forEach(phase => {
        ...
        const detailedTasks = atlas.task_breakdown?.detailed_tasks || [];
        const plannedHours = detailedTasks.length
            ? detailedTasks.reduce((sum, task) => sum + (Number(task.hours) || 0), 0)
            : Number(atlas.roadmap_simulator?.planned_hours || 0);

        // Math: capacity = team * hours * targetDays
        const capacity = team * hours * targetDays;

        const estDays = plannedHours > 0 ? Math.ceil(plannedHours / (team * hours)) : targetDays;
        ...
        if (messageEl) {
            if (onTrack) {
                ...
                messageEl.innerHTML = `
                    ...
                    <p>The plan requires <strong>${plannedHours.toFixed(1)} hours</strong> across <strong>${totalTasks}</strong> roadmap entries: about <strong>${estDays} days</strong>, within the <strong>${targetDays}-day</strong> limit (with a <strong>${buffer}-day</strong> buffer).</p>
                ...
                const minTeam = Math.ceil(plannedHours / (hours * targetDays));
                messageEl.innerHTML = `
                    <p style="margin-bottom: 8px; color: #f87171;"><strong>Simulation Status: AT RISK</strong></p>
                    <p>The team has <strong>${capacity.toFixed(1)} hours</strong>, but the planned work requires <strong>${plannedHours.toFixed(1)} hours</strong>.</p>
                    <p>Completing the plan will take about <strong>${estDays} days</strong>, resulting in a <strong>${overrun}-day</strong> delay.</p>
...
function initRiskDashboard(atlas) {
    ...
    if (alertsList) {
        ...
        let risks = [];
        if (atlas.risks && atlas.risks.length) {
            risks = atlas.risks;
        } else if (atlas.risk && atlas.risk.risks) {
        ...
        if (risks.length > 0) {
            risks.forEach(r => {
                ...
                const severity = r.severity || r.impact || "Warning";
                const sevColor = severity === "Critical" || severity === "High" ? "#f87171" : "var(--sunrise-amber)";
                card.innerHTML = `
                    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                        <span style="font-family: var(--font-code); color: var(--starlight); font-weight: 500;">${escapeHtml(r.title || r.category || "Production Risk")}</span>
                        <span style="font-family: var(--font-code); color: ${sevColor}; font-size: 11px; font-weight: bold; border: 1px solid ${sevColor}; padding: 1px 6px; border-radius: 4px;">${escapeHtml(severity)}</span>
                    </div>
                    <div style="color: rgba(240, 232, 208, 0.7); margin-bottom: 6px;">${escapeHtml(r.risk || r.description || "")}</div>
...
function renderVisualDependencyGraph(atlas) {
    if (window.updateDependencyGraphFlow) {
        window.updateDependencyGraphFlow(atlas);
    }
}
